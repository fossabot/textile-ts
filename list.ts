import type { StateABlockNode } from "./src/lib/pre-process/types.js";
import type { TextileNode, ElementNode, TextNode } from "./src/lib/types.js";
import { stateAAttributes } from "./src/lib/pre-process/attributes.js";
import { isPlainObject } from "./src/lib/helpers.js";
/**
...
export const parseList = (tree: (TextileNode | StateABlockNode)[]) => {
  ...
  for (const node of tree) {
    if (node.type === "lists") {
      ...
      let lines = node?.dataString?.split("\n");
      for (const line of lines as string[]) {
        let m1Obj = { start: "", reversed: false };
        let m: RegExpExecArray | null = null;
        ...
        if ((m = olMatchDot(line))) {
          ...
          newNode.properties = m[4]
            ? { ...m1Obj, ...stateAAttributes(m[4].trimStart()) }
            : { ...m1Obj };
          ...
          // Only apply m1Obj to newNode.properties if tagName is 'ol'
          props = m[4] ? { ...stateAAttributes(m[4].trimStart()) } : {};
          ...
          newNode.tagName = "ul";
          newNode.properties = m[2] ? stateAAttributes(m[2].trimStart()) : {};
        } else if ((m = ulMatch(line))) {
          newNode.tagName = "ul";
          props = m[2] ? stateAAttributes(m[2].trim()) : {};
          textValue = m[3];
          // Handle nesting
          if (m[1].length === 1) {
            liNode = {
