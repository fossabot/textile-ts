import { regexp, compile, escape_sc } from "./shares/re.js";
 ...
 * from `blacklist`
 */
export function copyAttr(s: { [x: string]: any }, blacklist?: { [x: string]: any }) {
  if (!s) {
    return undefined;
  }
...
function parseColgroup(src: string) {
...
export function parseTable(src: string, options?: Options) {
  ...
  do {
    ...
    // "rowgroup" (tbody, thead, tfoot)
      ...
      const tag = Constants.charToTag[m[1] as keyof typeof Constants.charToTag] || "tbody";
